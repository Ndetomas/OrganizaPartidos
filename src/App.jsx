...
// ^ Ve a Supabase → Project Settings → API Keys → Legacy → anon public → Copy
...
async function fetchReservations() {
  const { data: reservations, error: rErr } = await sb
    .from("reservations").select("*").order("date", { ascending: true });
  if (rErr) throw rErr;
  ...
  const { data: players, error: pErr } = await sb
    .from("players").select("*").order("sort_order");
  if (pErr) throw pErr;
  ...
  return reservations.map(r => {
    const rCourts = courts
      ...
      .map(c => ({
        ...
        payTo: c.pay_to,
        players: players.filter(p => p.court_id === c.id).map(p => p.name),
      }));
    ...
    return {
      ...
      courts: rCourts,
      pool: players.filter(p => p.reservation_id === r.id && p.status === "inscrito" && !p.court_id).map(p => p.name),
      waitlist: players.filter(p => p.reservation_id === r.id && p.status === "espera").map(p => p.name),
...
async function createReservation(form) {
  const { data: res, error } = await sb.from("reservations").insert({
    venue: form.venue, date: form.date,
    time_start: form.timeStart, time_end: form.timeEnd,
  }).select().single();
  if (error) throw error;

  const courts = Array.from({ length: parseInt(form.numCourts) }, (_, i) => ({
    reservation_id: res.id,
    name: `Pista ${parseInt(form.firstCourt) + i}`,
    price: parseFloat(form.price) || 6,
    pay_to: "", sort_order: i,
  }));
  const { error: cErr } = await sb.from("courts").insert(courts);
  if (cErr) throw cErr;
}
...
async function addCourt(resId, name, price) {
  const { error } = await sb.from("courts").insert({ reservation_id: resId, name, price, pay_to: "" });
  if (error) throw error;
}
...
async function addPlayer(resId, name, status = "inscrito") {
  const { error } = await sb.from("players").insert({ reservation_id: resId, name, status, court_id: null });
  if (error) throw error;
}

async function deletePlayer(resId, name) {
  const { error } = await sb.from("players").delete().eq("reservation_id", resId).eq("name", name);
  if (error) throw error;
}

async function movePlayer(resId, name, target) {
  const update = target === "waitlist" ? { court_id: null, status: "espera" }
    : target === "pool" ? { court_id: null, status: "inscrito" }
    : { court_id: target, status: "inscrito" };
  const { error } = await sb.from("players").update(update).eq("reservation_id", resId).eq("name", name);
  if (error) throw error;
}
...
async function applyReservationEdit(existing, form) {
  ...
  if (newCount > existing.courts.length) {
    const maxNum = existing.courts.reduce((m, c) => Math.max(m, parseInt(c.name.replace(/\D/g, "")) || 0), 0);
    for (let i = 0; i < newCount - existing.courts.length; i++)
      await addCourt(existing.id, `Pista ${maxNum + i + 1}`, parseFloat(form.price) || 6);
  } else if (newCount < existing.courts.length) {
    for (const c of existing.courts.slice(newCount))
...
:root{
  ...
  --ff:'Inter',sans-serif;--fd:'Syne',sans-serif;--r:14px;
}
...
.topbar h1{font-size:19px;color:var(--accent);line-height:1;}
...
.chip-del{background:none;border:none;color:var(--sub);font-size:18px;cursor:pointer;padding:4px 6px;border-radius:6px;min-width:32px;min-height:32px;display:flex;align-items:center;justify-content:center;}
...
.db-badge{display:inline-flex;align-items:center;gap:4px;padding:2px 7px;background:#22d68a10;border:1px solid #22d68a20;border-radius:20px;font-size:10px;color:var(--accent);font-weight:700;margin-left:6px;vertical-align:middle;}
`;
...
export default function App() {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      showToast("❌ Error al cargar — revisa tus credenciales Supabase");
      console.error(e);
    }
  }, [showToast]);
    ...
    <>
      ...
      <div className="app">
        <div className="topbar">
          ...
          <div>
            <h1>Pádel Manager <span className="db-badge">☁️ Live</span></h1>
        ...
        <div className="content">
          {tab === "home" && (
            <HomeTab upcoming={upcoming} past={past} loading={loading}
          ...
          {tab === "manage" && liveRes && (
            <ManageTab res={liveRes} withSave={withSave} showToast={showToast}
              setModal={(m) => setModal({...m, resId: liveRes.id})}
            />
          )}
        </div>

        {tab === "home" && <button className="fab" onClick={() => setModal({type:"res"})}>+</button>}
      ...
      {modal?.type === "res" && (
        <ResModal existing={modal.res} onClose={() => setModal(null)}
          onSave={(form) => {
            withSave(
              modal.res ? () => applyReservationEdit(modal.res, form) : () => createReservation(form),
              modal.res ? "Reserva actualizada ✓" : "Reserva creada ✓"
            );
            setModal(null);
          }}
        />
      ...
      {modal?.type === "move" && liveRes && (
        <MoveModal player={modal.player} source={modal.source} res={liveRes}
          onMove={(target) => {
            const label = target==="pool" ? "Disponibles" : target==="waitlist" ? "Lista de espera"
              : liveRes.courts.find(c=>c.id===target)?.name || target;
            withSave(() => movePlayer(liveRes.id, modal.player, target), `${modal.player} → ${label}`);
            setModal(null);
          }}
...
function HomeTab({ upcoming, past, loading, onOpen, onDelete, onEdit }) {
  if (loading) return <div className="loading-full"><div className="spinner"/> Conectando con Supabase…</div>;
  if (!upcoming.length && !past.length)
    return <p className="empty">No hay reservas.<br/>Pulsa <strong style={{color:"var(--accent)"}}>+</strong> para crear una.</p>;
  return (
    <>
      {upcoming.length > 0 && <><div className="sh"><span className="sh-title">Próximas</span></div>
        {upcoming.map(r=><ResCard key={r.id} res={r} onOpen={onOpen} onDelete={onDelete} onEdit={onEdit}/>)}</>}
      {past.length > 0 && <><div className="sh" style={{marginTop:8}}><span className="sh-title">Historial</span></div>
...
function ManageTab({ res, withSave, showToast, setModal }) {
  ...
  const handleAdd = () => {
    ...
    withSave(() => addPlayer(res.id, name, isFull?"espera":"inscrito"),
      ...
      <div className="card">
        ...
        <div className="row">
          <input ref={inputRef}
            ...
            placeholder="Nombre del jugador" value={playerName}
            onChange={e=>setPlayerName(e.target.value)}
            onKeyDown={e=>e.key==="Enter"&&handleAdd()}
          />
          <button className="btn btn-p" style={{flexShrink:0}} onClick={handleAdd}>{isFull?"Espera":"+ Añadir"}</button>
        </div>
        ...
        {res.pool.map(p=>(
          <div key={p} className="chip">
            ...
            <div className="row" style={{gap:6}}>
              ...
              <button className="chip-del" onClick={()=>withSave(()=>deletePlayer(res.id,p),`${p} eliminado`)}>✕</button>
            </div>
          </div>
        ))}
        {res.waitlist.length>0 && <>
          <hr/>
          <div className="sh"><span style={{fontSize:11,color:"var(--sub)",fontWeight:700,textTransform:"uppercase",letterSpacing:".5px"}}>Lista de espera ({res.waitlist.length})</span></div>
          {res.waitlist.map(p=>(
            <div key={p} className="chip chip-reserve">
              <div className="chip-name"><span>⏳</span><span>{p}</span></div>
              <div className="row" style={{gap:6}}>
                {!isFull && <button className="chip-movebtn" onClick={()=>withSave(()=>movePlayer(res.id,p,"pool"),`${p} promovido ✓`)}>▲ Promover</button>}
                <button className="chip-del" onClick={()=>withSave(()=>deletePlayer(res.id,p),`${p} eliminado`)}>✕</button>
              </div>
            </div>
          ))}
          ...
          <div className="court-fields">
            <div><label>Precio €</label>
              <input type="number" defaultValue={court.price}
                onBlur={e=>withSave(()=>updateCourt(court.id,{price:parseFloat(e.target.value)||0}))}/>
            </div>
            <div><label>A pagar a:</label>
              <input defaultValue={court.payTo} placeholder="Nombre"
                onBlur={e=>withSave(()=>updateCourt(court.id,{payTo:e.target.value}))}/>
            </div>
          </div>
        ...
        <ul className="move-list">
          {opts.map(o=>(
            <li key={o.id} className={`move-item ${o.dis?"dis":""}`} onClick={()=>!o.dis&&onMove(o.id)}>
              <div><div style={{fontWeight:600}}>{o.label}</div><div style={{fontSize:11,color:"var(--sub)",marginTop:2}}>{o.sub}</div></div>
...
function WAModal({ res, onClose, showToast }) {
  ...
  const copyText = async () => {
    const text=getText();
    try {
      if (navigator.clipboard?.writeText) { await navigator.clipboard.writeText(text); showToast("📋 Copiado ✓"); return; }
    } catch {}
    try {
...
function ResModal({ onClose, onSave, existing }) {
  ...
  const [form,setForm]=useState(existing?{
    ...
    firstCourt:existing.courts[0]?.name.replace(/\D/g,"")||"14",
    price:String(existing.courts[0]?.price||6),
  }:{venue:"",date:today,timeStart:"18:30",timeEnd:"20:00",numCourts:"2",firstCourt:"14",price:"6"});
  ...
  return (
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="sheet">
        ...
        <div className="fg2">
          <div className="field"><label>Hora inicio</label><input type="time" value={form.timeStart} onChange={e=>set("timeStart",e.target.value)}/></div>
          <div className="field"><label>Hora fin</label><input type="time" value={form.timeEnd} onChange={e=>set("timeEnd",e.target.value)}/></div>
        </div>
        <div className="fg3">
          <div className="field"><label>Nº pistas</label><input type="number" min="1" max="10" value={form.numCourts} onChange={e=>set("numCourts",e.target.value)}/></div>
          <div className="field"><label>1ª pista nº</label><input type="number" min="1" value={form.firstCourt} onChange={e=>set("firstCourt",e.target.value)}/></div>
          <div className="field"><label>Precio €</label><input type="number" step="0.5" value={form.price} onChange={e=>set("price",e.target.value)}/></div>
        </div>
